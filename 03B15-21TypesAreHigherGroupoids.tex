...
\usepackage{pmmeta}
\pmcanonicalname{21TypesAreHigherGroupoids}
\pmcreated{2013-11-19 22:56:27}
\pmmodified{2013-11-19 22:56:27}
\pmowner{PMBookProject}{1000683}
\pmmodifier{rspuzio}{6075}
\pmtitle{2.1 Types are higher groupoids}
\pmrecord{13}{87598}
\pmprivacy{1}
...
\renewcommand{\thepredefn}{2.1.\arabic{mathcount}}
\newenvironment{myeqn}{\begin{equation}}{\end{equation}\addtocounter{mathcount}{1}}
\renewcommand{\theequation}{2.1.\arabic{mathcount}}
\newtheorem{prelem}{Lemma}
\newenvironment{lem}{\begin{prelem}}{\end{prelem}\addtocounter{mathcount}{1}}
\renewcommand{\theprelem}{2.1.\arabic{mathcount}}
...
\renewcommand{\theprermk}{2.1.\arabic{mathcount}}
\newtheorem{prethm}{Theorem}
\newenvironment{thm}{\begin{prethm}}{\end{prethm}\addtocounter{mathcount}{1}}
\renewcommand{\theprethm}{2.1.\arabic{mathcount}}
\let\autoref\cref
\let\type\UU
\makeatother
...
Thus, the statement of a lemma or theorem should be translated into a type, as in \PMlinkname{\S 1.11}{111propositionsastypes}, and its proof translated into an inhabitant of that type.
According to the interpretation of the universal quantifier ``for every'', the type corresponding to \PMlinkname{Lemma 2.1.1}{21typesarehighergroupoids#Thmprelem1} is
\[ \prd{A:\UU}{x,y:A} (x= y)\to(y= x). \]
The proof of \PMlinkname{Lemma 2.1.1}{21typesarehighergroupoids#Thmprelem1} will consist of constructing an element of this type, i.e.\ deriving the judgment $f:\prd{A:\UU}{x,y:A} (x= y)\to(y= x)$ for some $f$.
...
(As remarked in \PMlinkname{\S 1.1}{11typetheoryversussettheory}, the secondary statement ``$\opp{\refl{x}}\jdeq\refl{x}$ for each $x:A$'' should be regarded as a separate judgment.)
...
\begin{proof}[First proof]
  ...
  The conversion rule~\PMlinkname{(2.0.1)}{2homotopytypetheory#S0.E1} gives $\opp{\refl{x}}\jdeq \refl{x}$, as required.
\end{proof}
...
To be even more formal, we could say that \PMlinkname{Lemma 2.1.1}{21typesarehighergroupoids#Thmprelem1} and its proof together consist of the judgment
...
\PMlinkname{Lemma 2.1.2}{21typesarehighergroupoids#Thmprelem2} has three obvious proofs: we could do induction over $p$, induction over $q$, or induction over both of them.
If we proved it three different ways, we would have three different elements of the same type.
It's not hard to show that these three elements are equal (see \PMlinkexternal{Exercise 2.1}{http://planetmath.org/node/87632}), but as they are not \emph{definitionally} equal, there can still be reasons to prefer one over another.

In the case of \PMlinkname{Lemma 2.1.2}{21typesarehighergroupoids#Thmprelem2}, the difference hinges on the computation rule.
...
(We choose left-associativity for concreteness, although in view of \PMlinkname{Lemma 2.1.4}{21typesarehighergroupoids#Thmprelem3}(\ref{item:omg4}) below it makes litle difference.)
...
\begin{proof}[Proof of~\PMlinkname{Lemma 3}{21typesarehighergroupoids#Thmlem3}]
...
\begin{rmk}
  ...
  For instance, in \PMlinkname{Lemma 2.1.4}{21typesarehighergroupoids#Thmprelem3}(\ref{item:omg4}) we might do induction only over one or two paths rather than all three.
...
In view of \PMlinkname{Lemma 2.1.4}{21typesarehighergroupoids#Thmprelem3}(\ref{item:omg4}), we will often write $p\ct q\ct r$ for $(p\ct q)\ct r$, and similarly $p\ct q\ct r \ct s$ for $((p\ct q)\ct r)\ct s$ and so on.
We choose left-associativity for definiteness, but it makes no real difference.
We generally trust the reader to insert instances of \PMlinkname{Lemma 2.1.4}{21typesarehighergroupoids#Thmprelem3}(\ref{item:omg4}) to reassociate such expressions as necessary.
...
(see \PMlinkexternal{Exercise 2.4}{http://planetmath.org/node/87635}), but we will not need it.
...
\begin{figure}
 \centering
 \includegraphics{HoTT_fig_2.1.1.png}
\end{figure}
%\[
% \xymatrix@+5em{
...
where $\mathsf{ru}_p : p = p \ct \refl{b}$ is the right unit law from \PMlinkname{Lemma 2.1.4}{21typesarehighergroupoids#Thmprelem3}(\ref{item:omg1}).
...
The foregoing fact, which is known as the \emph{Eckmann--Hilton argument}, comes from classical homotopy theory, and indeed it is used in \PMlinkexternal{Chapter 8}{http://planetmath.org/node/87582} below to show that the higher homotopy groups of a type are always abelian\index{group!abelian} groups.
...
We will return to iterated loop spaces in \PMlinkexternal{Chapter 7}{http://planetmath.org/node/87580},\PMlinkexternal{Chapter 6}{http://planetmath.org/node/87579},\PMlinkexternal{Chapter 8}{http://planetmath.org/node/87582}.
\index{.infinity-groupoid@$\infty$-groupoid!structure of a type|)}%
\index{type!identity|)}
\index{path|)}%
