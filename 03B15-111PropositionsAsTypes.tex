...
\usepackage{pmmeta}
\pmcanonicalname{111PropositionsAsTypes}
\pmcreated{2013-11-13 20:24:58}
\pmmodified{2013-11-13 20:24:58}
\pmowner{PMBookProject}{1000683}
\pmmodifier{rspuzio}{6075}
\pmtitle{1.11 Propositions as types}
\pmrecord{24}{87547}
\pmprivacy{1}
...
\renewcommand{\theequation}{1.11.\arabic{mathcount}}
\let\autoref\cref
\let\nat\N
\makeatother
...
\begin{myeqn}\label{eq:tautology1}
...
\begin{myeqn}\label{eq:tautology2}
...
For example, while by default there is no construction witnessing \LEM{}, the logic is still compatible with the existence of one (see \PMlinkname{\S 3.4}{34classicalvsintuitionisticlogic}).
...
See \PMlinkexternal{Exercise 1.12}{http://planetmath.org/node/87565},\PMlinkexternal{Exercise 1.13}{http://planetmath.org/node/87566} for some suggestions.
\index{logic!classical vs constructive|)}
...
The former is more natural in constructive mathematics, but in this case it is actually equivalent to the latter, since $\nat$ has ``decidable equality'' (see \PMlinkname{\S 3.4}{34classicalvsintuitionisticlogic},\PMlinkname{Theorem 7.2.6}{72uniquenessofidentityproofsandhedbergstheorem#Thmprethm4}).
\index{decidable!equality}%

The representation of propositions as types also allows us to incorporate axioms into the definition of types as mathematical structures using $\Sigma$-types, as discussed in \PMlinkname{\S 1.6}{16dependentpairtypes}.
...
We will return to this example in \PMlinkname{\S 2.14}{214exampleequalityofstructures}.
...
We will return to this issue in \PMlinkname{\S 3.5}{35subsetsandpropositionalresizing}.
...
This is to be distinguished from the stronger notion of \emph{equivalence of types} to be introduced in \PMlinkname{\S 2.4}{24homotopiesandequivalences},\PMlinkexternal{Chapter 4}{http://planetmath.org/node/87577}:
although $\mathbb{N}$ and $\unit$ are logically equivalent, they are not equivalent types.

In \PMlinkexternal{Chapter 3}{http://planetmath.org/node/87576} we will introduce a class of types called ``mere propositions'' for which equivalence and logical equivalence coincide.
